...
class UserController {
    ...
    async addPage(req, res) {
        ...
        return res.render('users/add',{specialities});
    }

    async add(req, res) {
        try{
            let organization = req.user
            let { fullName, email, countryCode, phoneNumber, password, specality } = req.body;
            ...
            if (currentUsers.length >= organization.numAccount) {
                req.flash('error', req.__('User limit exceeded. please contact your administrator.'));
    ...
    async editPage(req, res) {
        ...
        return res.render('users/edit', {
            userFetch,specialities
        });
    }

    async edit(req, res) {
        const { fullName, email, phoneNumber,specality } = req.body;
...
async function getSpecialities(){
    let x =  await Speciality.find({
        "isSuspended" : false,
        "isDeleted" : false,
    }).select("_id specialityName").lean();

    return _.sortBy(x, ['specialityName']);
