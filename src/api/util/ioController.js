...
const {
    models: { User,Coach,Chat,Message, FriendRequest,Referral },
} = require('./../../../lib/models');
...
//module.exports.listen = function(server) {
    ...
    io.on('connection', async function(socket) {
        ...
        //console.log('log input param : ' + socket.handshake.query.token);
        ...
        if (verifyUserResConnect.error) {
            ...
            socket.join(verifyUserResConnect.data.user._id.toString(), () => {   })
            
            socket.emit('connected', {});
            console.log('connected')
            if (process.env.NODE_ENV === 'production') {
                pubClient.set("user:"+verifyUserResConnect.data.user._id.toString(), socket.id);
            }
        }
        ...
        socket.on('user-login', async function(data, ack) {
            ...
            console.log("verifyUserRes.data.user._id.toString()",verifyUserRes.data.user._id.toString())
            socket.join(verifyUserRes.data.user._id.toString(), () => {   })

            if (process.env.NODE_ENV === 'production') {
                pubClient.set("user:"+verifyUserRes.data.user._id.toString(), socket.id);
            }
            return ack({
                status: 'success',
            });
        ...
        socket.on('user-logout', async function(data, ack) {
            ...
            if (process.env.NODE_ENV === 'production') {
                pubClient.del("user:"+verifyUserRes.data.user._id)
            }
            return ack({
                status: 'success',
            });
        ...
        socket.on('disconnect', async function() {
            ...
            if (process.env.NODE_ENV === 'production') {
                pubClient.del("user:"+userLoginKey)
            }
            // eslint-disable-next-line no-console
        ...
        socket.on('message', async function(data, ack) {
            const { token, language, receiver_id, message ,message_type, file,file_type  } = data;
        ...
        //   connected_id =connected_id.toString()
            ...
            if( message_type ==='TABLE' ){
                try{
                    ...
                    referral.senderId = verifyUserRes.data.user._id;
                    referral.receiverId = receiver_id
                    referral.message = message;
                    referral.msgTime = moment().utc().unix()

                    if( receiverExist.organizationId ){
                        referral.organizationId = receiverExist.organizationId;
                    }
            ...
            let receiver_skt;
            if (process.env.NODE_ENV === 'production') {
                receiver_skt = await pubClient.get("user:"+receiver_id)
            }
            
            if (receiver_skt) {
                io.to(receiver_id).emit("message",msg);
            }
            else if(connect==verifyUserRes.data.user._id.toString()){
                io.to(receiver).emit("message",msg);
            }else{
                if( process.env.OS && process.env.OS === 'window' ){
                    //do nothing
                }else{
                    const themis = require('jsthemis')
                    let clientPrivateKey =   process.env.ENC_PUBLIC_KEY;
                    let serverPublicKey =    process.env.ENC_PRIVATE_KEY;
                    let sm = new themis.SecureMessage(new Buffer.from(clientPrivateKey, "base64"), new Buffer.from(serverPublicKey, "base64"));
                    let decryptedString = ""
                    
                    if( message ){
                        if( message_type ==='TABLE' ){
                            decryptedString = "New message"
                        }else
                            decryptedString =  sm.decrypt(new Buffer.from(message, "base64")).toString();
                    }
                    
                    verifyUserRes.data.user.fullName = `Dr. ${verifyUserRes.data.user.fullName}`
            ...
            .sort({updated:-1});
              ...
              if(chatlist.length>0){
                ...
                for(let i=0;i<=lng-1;i++){
                    ...
                    obj['message_type'] = ''
                    if( lastMessage ){
                        if( lastMessage.message_type=='NORMAL' || lastMessage.message_type=='TABLE' ){
                            obj['last_message'] = lastMessage.message;
                        }else{
                            obj['last_message'] = lastMessage.file;
                        }
        ...
        socket.on('block_user', async function(data, ack) {
            ...
            if(receiver){
                ...
                io.to(receiver_id).emit("block_user",sender.user);
            }
        ...
        socket.on('unblock_user', async function(data, ack) {
            ...
            if(receiver){
                ...
                io.to(receiver_id).emit("unblock_user",sender.user);
            }
        ...
        socket.on('request-both', async function(data, ack){
            console.log("hhihihi")
            const { token, language, type} = data;
            console.log({ token, language, type})
            const __ = withLanguage(language);
            ...
            //if(type == "REQUEST"){
            let requsetListRequest = await Chat.find({
                    user_id: verifyUserRes.data.user._id, is_approve: false, is_reject: false
                }).populate({ 'path':'receiver_id', 'select':'fullName avatar' }).populate({ 'path':'user_id',  'select':'fullName avatar'}).lean();


            requsetListRequest = requsetListRequest.map(x => {
                x['type'] = "REQUEST"; // Assign the value to the type property
                return x; // Return the modified object
            });

            //}else{
            let requsetListBoth = await Chat.find({
                    receiver_id: verifyUserRes.data.user._id, is_approve: false, is_reject: false
                }).populate({ 'path':'user_id',  'select':'fullName avatar'}).populate({ 'path':'receiver_id',  'select':'fullName avatar'}).lean();
            //}

            //requsetListBoth = requsetListBoth.map( x=> x['type']=type )
            requsetListBoth = requsetListBoth.map(x => {
                x['type'] = "RECEIVED"; // Assign the value to the type property
                return x; // Return the modified object
            });
            ...
            console.dir(requsetList)

            return ack({
                status: 'success',
                requsetList:allRequest
            });
        });
